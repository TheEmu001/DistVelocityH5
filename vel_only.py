...
def velocity(video, color, label):
    ...
    all_data[video + "_vel"] = vel_avg[59:108060]
    all_data[video+"_dist"] = dist_sum[59:108060]
    time = np.arange(len(all_data[video + "_vel"])) * 1. / fps  # time that is 1/60 sec
    all_data['time'] = time / 60
if __name__ == '__main__':
    ...
    better_time_ns = (vel_only_df.index)*(1./fps)/60
